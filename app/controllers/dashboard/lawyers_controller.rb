class Dashboard::LawyersController < DashboardController
  before_action :set_lawyer, only: [:show, :edit, :update, :destroy]

  # GET /lawyers
  # GET /lawyers.json
  def index
    @lawyers = Dashboard::Lawyer.all
  end
  ...
  # POST /lawyers.json
  def create
    @lawyer = Dashboard::Lawyer.new(lawyer_params)
  ...
  private
    ...
    def set_lawyer
      @lawyer = Dashboard::Lawyer.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def lawyer_params
      params.require(:dashboard_lawyer).permit(:name)
    end
end
